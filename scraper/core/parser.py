...
def _format_publish_time(raw_time) -> str:
    """Convert Upwork publishTime (ISO string or epoch ms) to a human-readable string."""
    if not raw_time:
        return "Unknown"
    try:
        # Upwork returns ISO 8601 strings like "2024-04-22T08:30:00+0000"
        if isinstance(raw_time, str):
            dt = datetime.datetime.fromisoformat(raw_time.replace("Z", "+00:00"))
        else:
            # Fallback: treat as milliseconds epoch
            dt = datetime.datetime.utcfromtimestamp(int(raw_time) / 1000)
        # How long ago
